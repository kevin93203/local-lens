...
索引會寫入 app-data 的 `index.sqlite3`，保存圖片的路徑、檔案大小、修改時間、縮圖、OCR 結果、CLIP 向量與人臉群組資料。重新選擇同一資料夾時，程式會以檔案大小與修改時間判斷是否有變更：未變更的圖片直接重用 SQLite 快取，不會重新產生縮圖、執行 OCR 或模型推論；新增或變更的檔案才會重建索引。介面進度與完成訊息會顯示重用的快取筆數。
